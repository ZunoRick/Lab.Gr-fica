...
float posX = 0, posY = 2.5, posZ = -3.5, rotRodIzq = 0, rotRodDer = 0, rotBraIzq = 0, rotBraDer = 0, rotCue = 0, rotBraDerZ = 0, rotBraIzqZ = 0;
float rotCuello = 0, rotPiernaDer = 0, rotPiernaIzq = 0, rotCintura = 0;
float giroMonito = 0;
...
typedef struct _frame
{
	...
	float giroMonitoInc;
	float rotRodDerInc; //variable para el incremento de la rodilla derecha 
	float rotBraIzqInc;
	float rotBraDerInc;
	float rotBraDerZInc;
	float rotBraIzqZInc;
	float rotCuelloInc;
	float rotPiernaDerInc;
	float rotPiernaIzqInc;
	float rotCinturaInc;
}FRAME;
...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].rotCintura = rotCintura;
	FrameIndex++;
}
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].rotBraDerZInc = (KeyFrame[playIndex + 1].rotBraDerZ - KeyFrame[playIndex].rotBraDerZ) / i_max_steps;
	KeyFrame[playIndex].rotBraIzqZInc = (KeyFrame[playIndex + 1].rotBraIzqZ - KeyFrame[playIndex].rotBraIzqZ) / i_max_steps;
	KeyFrame[playIndex].rotCuelloInc = (KeyFrame[playIndex + 1].rotCuello - KeyFrame[playIndex].rotCuello) / i_max_steps;
	KeyFrame[playIndex].rotPiernaDerInc = (KeyFrame[playIndex + 1].rotPiernaDer - KeyFrame[playIndex].rotPiernaDer) / i_max_steps;
	KeyFrame[playIndex].rotPiernaIzqInc = (KeyFrame[playIndex + 1].rotPiernaIzq - KeyFrame[playIndex].rotPiernaIzq) / i_max_steps;
	...
	glPushMatrix();//Pecho
		...
		glPushMatrix(); //Brazo derecho-->
			...
			glPushMatrix();
				...
				glRotatef(rotBraDer, 0, 1, 0);
				glRotatef(rotBraDerZ, 0, 0, 1);
		...
		glPushMatrix(); //Brazo izquierdo <--
			...
			glPushMatrix();
				glTranslatef(-0.25, 0, 0);
				glRotatef(rotBraIzq, 0, 1, 0);
				glRotatef(rotBraIzqZ, 0, 0, 1);
				glTranslatef(-0.75, 0, 0);
				fig7.prisma(0.7, 1.5, 0.7, 0);
			glPopMatrix();
		glPopMatrix();
...
void InitGL(GLvoid)     // Inicializamos parametros
{
	...
	for (int i = 0; i<MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].rotBraDerZInc = 0;
		KeyFrame[i].rotBraIzqZInc = 0;
		KeyFrame[i].rotPiernaDer = 0;
		KeyFrame[i].rotPiernaDerInc = 0;
		KeyFrame[i].rotPiernaIzq = 0;
		KeyFrame[i].rotPiernaIzqInc = 0;
...
void animacion()
{
	...
	if (play)
	{
		...
		else
		{
			...
			rotBraDerZ += KeyFrame[playIndex].rotBraDerZInc;
			rotBraIzqZ += KeyFrame[playIndex].rotBraIzqZInc;
			giroMonito += KeyFrame[playIndex].giroMonitoInc;
			rotCuello += KeyFrame[playIndex].rotCuelloInc;
			rotPiernaDer += KeyFrame[playIndex].rotPiernaDerInc;
			rotPiernaIzq += KeyFrame[playIndex].rotPiernaIzqInc;
